...
public sealed class CaptureZoneEntity : Component, Component.IDamageable, Component.INetworkListener
{
	...
	protected override void OnUpdate()
	{
		...
		// 	.Where( e => e.GameObject.Tags.Has( "player" ) && Vector3.Distance( e.Transform.Position, Transform.Position ) <= CaptureRadius )
		// 	.ToList();
		//
	...
	{
		// You can add visual feedback or other logic here when health changes
	}

	private void OnPointCaptured()
	{
		Log.Info( $"{PointName} has been captured by {ControllingTeam}!" );
		// Add any additional logic for when a point is captured
	}

	private void OnPointNeutralized()
	{
		Log.Info( $"{PointName} has been neutralized!" );
		// Add any additional logic for when a point is neutralized
	}

	private void OnPointDestroyed()
	{
		Log.Info( $"{PointName} has been destroyed!" );
		// Add any additional logic for when a point is destroyed
		GameObject.Destroy();
	}
}
